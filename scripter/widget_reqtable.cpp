...
class widget_reqtable : public controls::table {
	...
	void* addrow() override {
		setmodified();
		return source_array.addz();
	}
	int getmaximum() const override {
		return source_array.getcount();
	}
	void* get(int index) const override {
		return source_array.ptr(index);
	}
...
public:
	widget_reqtable(const requisit* v) : source(v), source_array(v->getlenght()), modified(false) {
...
bool requisit::edit() {
	widget_reqtable table(this);
	if(!controls::edit(table))
		return false;
	return true;
}
